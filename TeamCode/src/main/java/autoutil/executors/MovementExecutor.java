...
//                }
            ...
            if (paths.get(curPath).size() == curPose) {
                ...
                double dis = sqrt(pow(bot.odometry2.getX() - nextPose.p.x, 2)
                        + pow(bot.odometry2.getY() - nextPose.p.y, 2));
                if (dis > 10) {
                    move(
                        forwardPow,
    ...
    private boolean doneWithSetpoint() {
        ...
        return abs(reactor.turnPow(nextPose.ang, startH, true)) < 0.4
                && sqrt(pow(bot.odometry2.getX() - nextPose.p.x, 2)
                + pow(bot.odometry2.getY() - nextPose.p.y, 2)) < 4;
    }
